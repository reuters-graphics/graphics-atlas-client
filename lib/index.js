...
// Available topojson resolutions at the topojson/ root. '50m' is the default:
// it's the more detailed set AND covers more geographies than '110m'. Any
// geography missing the requested resolution falls back to the complete,
// resolution-less topojson/custom/ set.
const RESOLUTIONS = ['50m', '110m'];
const DEFAULT_RESOLUTION = '50m';
...
class MetadataClient {
  ...
  // Data fetchers
  async fetchGlobalTopojson(resolution = DEFAULT_RESOLUTION) {
    return this._fetchGeography('world', resolution);
  }

  async fetchRegionTopojson(regionNameOrSlug, resolution = DEFAULT_RESOLUTION) {
    const region = this.getRegion(regionNameOrSlug);
    if (!region) throw new Error(`No region found for: ${regionNameOrSlug}`);
    return this._fetchGeography(region.slug, resolution);
  }

  async fetchSubregionTopojson(subregionNameOrSlug, resolution = DEFAULT_RESOLUTION) {
    ...
    return this._fetchGeography(subregion.slug, resolution);
  }

  async fetchCountryTopojson(nameSlugOrCode, resolution = DEFAULT_RESOLUTION) {
    const country = this.getCountry(nameSlugOrCode);
    if (!country) throw new Error(`No country found for: ${nameSlugOrCode}`);
    return this._fetchGeography(country.isoAlpha2, resolution);
  }

  // Fetches `<key>.<resolution>.json`, falling back to the complete,
  // resolution-less `custom/<key>.json` when that resolution isn't published
  // for the geography (e.g. countries that only ship one resolution).
  async _fetchGeography(key, resolution) {
    if (!RESOLUTIONS.includes(resolution)) {
      throw new Error(`Invalid resolution "${resolution}". Use one of: ${RESOLUTIONS.join(', ')}`);
    }
    const primary = `/topojson/${key}.${resolution}.json`;
    const primaryRes = await fetch(FETCH_BASE + primary);
    if (primaryRes.ok) return primaryRes.json();

    const fallback = `/topojson/custom/${key}.json`;
    const fallbackRes = await fetch(FETCH_BASE + fallback);
    if (fallbackRes.ok) return fallbackRes.json();

    throw new Error(
      `Failed to fetch topojson for "${key}" ` +
      `(${resolution}: ${primaryRes.status}, custom: ${fallbackRes.status})`
    );
  }
}
